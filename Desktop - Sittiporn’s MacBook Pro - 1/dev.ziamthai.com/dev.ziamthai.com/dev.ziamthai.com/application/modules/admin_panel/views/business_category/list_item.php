...
<?php /*  echo "<pre>"; print_r($data); print_r($parent); echo "</pre>";  */?>
<?php 
	$sBackClick = sprintf("%s", base_url('index.php/admin_panel/business_category/')); 
//	$sTitleBack = sprintf(" Biz Category '%s' ",$parent['bsth_title']);
	$sAddClick = sprintf("%s%d", base_url('index.php/admin_panel/business_category/add_item/?&id='),$parent['bsth_id']); 
//	$sTitleAdd = "Add Sub Category in ".$parent['bsth_title'];
	$sTitleBack = "Food Category";
	$sTitleAdd = "Add Sub Food Category";
?>
		...
		<div class="card-header">
 			<div class="row">
					<div class="col-md-9 col-md-right h3">
						<!--
						<a href="<?php echo $sBackClick; ?>" class="btn btn-primary" title="<?php echo $sTitleBack; ?>">
						<i class="fa fa-chevron-left"></i> Back </a> -->
						<span class="top10"><?php echo $sTitleBack; ?><span>
					</div>
...
<?php } ?>
		...
		<div class="card-block">
				...
				foreach ($data['data'] as $info) {
					...
					$sEditClick = sprintf("%s?id=%d", base_url('index.php/admin_panel/business_category/edit_item/'), $recID);
					$sDeleteClick = sprintf("%s?id=%d", base_url('index.php/admin_panel/business_category/delete_item/'), $recID);
					$sRestoreClick = sprintf("%s?id=%d", base_url('index.php/admin_panel/business_category/restore_item/'), $recID);
				...
				<div class="row <?php echo $sClassRow; ?>">
						...
						<div class="col-md-4 text-md-right top05">
								<nowrap>
...
<?php } ?>
				...
				if ($curr>=2) {
					$clsPrev='';
					$lnkPrev=base_url('index.php/admin_panel/business_category/list_item/?id='.$parent['bsth_id'].'&curr=1');
					$lnkPrev.=sprintf("&ppage=%d",$ppage);
				}
				if ($data['index']['curr']+1 <= $maxPage) {
					$clsNext='';
					$lnkNext=base_url('index.php/admin_panel/business_category/list_item/?id='.$parent['bsth_id'].'&curr='.$maxPage);
...
<?php for ($iRun=$iStart;$iRun<=$iFin; $iRun++) {
	$lnkPage=base_url('index.php/admin_panel/business_category/list_item/?id='.$parent['bsth_id']);
