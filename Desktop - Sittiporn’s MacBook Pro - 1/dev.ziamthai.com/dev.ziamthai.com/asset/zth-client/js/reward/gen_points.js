...
$(function () {
    ...
    $('#gen_points_form').submit(function (e) {
        e.preventDefault();
        ...
        $.ajax({
            type: 'POST',
            url: api_url + 'reward/generate_point',
            data: $(this).serialize(),
            dataType: 'JSON',
            success: function (resp) {

                $('#gen_points_result').html(resp.message);

                if (resp.error) {
                    $('#gen_points_form').slideDown();
                }

                if (resp.success) {
                    $('#gen_points_result').removeClass('alert-danger').addClass('alert-success');

                    $('#member_photo').attr('src', base_url + 'index.php/uploads/user_photo/large/0');
                    $('#member_points_active').html(0);

                    $('#input_order_amount').val(0);
                    $('#input_order_no').val('');
                    $('#input_points_amount').val(0);

                    $('#gen_points_form').find('input[name=member_id]').val(0);
                    $('#gen_points_form').find('input[name=member_name]').val('');
                    $('#gen_points_form').find('input[name=member_email]').val('');
                    $('#gen_points_form').fadeOut();
    ...
    $('#modal_reward_history').on('show.bs.modal', function (event) {
        var member_id = parseInt($('#gen_points_form').find('input[name=member_id]').val()) || false;
...
function member_search_set_points(data) {
    ...
    $('#gen_points_result').hide();

    $('#member_photo').attr('src', base_url + 'index.php/uploads/user_photo/large/' + data.id);

    $('#member_points_active').html(data.reward_points);

    $('#gen_points_form').find('input[name=member_id]').val(data.id);
    $('#gen_points_form').find('input[name=member_name]').val(data.name);
    $('#gen_points_form').find('input[name=member_email]').val(data.email);
    $('#gen_points_form').fadeIn();
}

function exchange_rate_cal_points(exc_rate) {
    var rate = parseFloat(exc_rate) || 0;

    var bill_amount = parseFloat($('#input_order_amount').val()) || 0;

    var points_amount = (bill_amount / rate);

    $('#input_points_amount').val(parseInt(points_amount));
}
