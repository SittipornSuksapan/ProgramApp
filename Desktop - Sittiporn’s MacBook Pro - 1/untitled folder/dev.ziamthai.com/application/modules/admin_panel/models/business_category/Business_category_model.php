...
//			$this->db->set('bssi_title', $desc);
		...
		public function api_list_data($curr=1,$ppage=10,$id=101) {
			$result=array('result'=>false,'message'=>null,'data'=>null);
			$result['index']=array("max"=>0,"curr"=>1,"ppage"=>10,"start"=>0,"stop"=>0,"next"=>0,"prev"=>0);
			$this->db->from('business_type');
			$this->db->where('bsth_pid', $id);
			$this->db->order_by('bsth_title ASC');
			$query = $this->db->get();
		...
		public function list_parent($curr=1,$ppage=10,$id=0) {
			$result=array('result'=>false,'message'=>null,'data'=>null);
			$result['index']=array("max"=>0,"curr"=>1,"ppage"=>10,"start"=>0,"stop"=>0,"next"=>0,"prev"=>0);
			$this->db->from('business_type');
