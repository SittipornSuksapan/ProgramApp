...
$(function () {
    ...
    $("#reviews_photo_list").on("click", ".photo_item_btn", function () {
        ...
        var img_type = form_reviews_photos[photo_id].bsri_imgt_id || 0;
        var img_caption = form_reviews_photos[photo_id].bsri_caption || '';
...
function reviews_photo_upload_result(resp) {
    ...
    if (resp.photo_data) {
        reviews_photo_add(resp.photo_data);
    }
...
function reviews_photo_del(id) {
    ...
    $('#reviews_writing_form').children('.photo_item_' + get_id).remove();
...
function reviews_photo_add(data) {
    var photo_id = parseInt(data.bsri_id) || false;
    ...
    if ($('#reviews_photo_list').children('.photo_item[rel=' + photo_id + ']').length < 1) {
        $('#reviews_writing_form').prepend('<input class="photo_item_' + photo_id + '" type="hidden" name="photo[]" value="' + photo_id + '">');

        $('#reviews_photo_list').append('<div class="photo_item col-xs-6 col-md-3" rel="' + photo_id + '"><a href="javascript:void(0);" class="photo_item_btn btn btn-secondary" rel="' + photo_id + '"><img src="' + set_img_url + '"></a><a href="javascript:reviews_photo_del(\'' + photo_id + '\');" class="photo_item_del btn btn-danger"><i class="fa fa-close"></i></a></div>');
    } else {
        $('#reviews_photo_list').children('.photo_item[rel=' + photo_id + ']').find('.photo_item_btn').html('<img src="' + set_img_url + '">');
    }
